...
class Parser(object): 
    ...
    def parse_sentence(self, words, pos):
        ...
        while state.buffer: 
            ...
            pred = self.model.predict_on_batch(np.array([feature,]))
            i = -1
            sorted = np.argsort(pred)
            while True:
                index = sorted[-1][i]
                transition = self.output_labels[index]
                ...
                elif operation=='shift'  and len(state.buffer) == 1 and len(state.stack) !=0:
                    i-=1
                    continue
                #the root node must never be target of left-arc 
                elif operation=='left_arc' and label == 'root':
                    i-=1 
                    continue
                break
            if operation=='left_arc':
                ...
                print("No legal operation permitted")
